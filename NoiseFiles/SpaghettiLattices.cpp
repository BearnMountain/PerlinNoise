...
public:
    ...
    SpagettiLattices(const uint16_t MAP_WIDTH, const uint16_t MAP_HEIGHT, uint8_t* noisemap, const uint8_t frequency, const unsigned int seed)
        ...
        for(uint8_t i=0;i<frequency;i++){
            for(uint8_t j=0;j<frequency;j++){
                ...
                noisemap[(y1+i)*MAP_WIDTH+(x1+j)] = static_cast<uint8_t>(
                    std::abs(smoothStep( // only change from perlin noise
